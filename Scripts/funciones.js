...
const mostrarProductosNombre = (ubicacion,clasificador,ubicacionCantidad) => {
    productosServicios.listaProductos().then((respuesta) => {
        respuesta.forEach(producto => {
            if(producto.name.toLowerCase().includes(clasificador)){
                ubicacion.appendChild(productosServicios.crearNuevaLinea(name.imageUrl,price,id,alt,section,description))
            }
        })
